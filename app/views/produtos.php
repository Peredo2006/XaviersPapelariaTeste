                    ...
                    <?php foreach ($produtos as $p): ?>
                        <div class="produto-card">
                            <div class="produto-foto">
                                <?php if (!empty($p['foto_produto']) && file_exists(__DIR__ . '/../../public/uploads/' . $p['foto_produto'])): ?>
                                    <img src="../../public/uploads/<?php echo e($p['foto_produto']); ?>" alt="">
                                <?php else: ?>
                                    <div style="font-weight:700;color:#222;">FOTO DO PRODUTO</div>
                                <?php endif; ?>
                            </div>

                            <div class="produto-nome"><?php echo e($p['nome_produto']); ?></div>
                            <div class="produto-valor">R$ <?php echo number_format((float)$p['preco_produto'], 2, ',', '.'); ?></div>
        ...
        <div class="modal-box" role="dialog" aria-modal="true">
            ...
            <form id="frmProduto" method="post" enctype="multipart/form-data" action="../models/processa_produto.php?acao=novo">
                ...
                <div class="form-row">
                    ...
                    <input type="number" name="quantidade_produto" id="quantidade_produto" min="0" value="0" required>
                </div>
                <div class="form-row">
                    <label>Preço</label>
                    <input type="number" step="0.01" name="preco_produto" id="preco_produto" min="0" value="0.00" required>
                </div>
                <div class="form-row">
                    <label>Descrição</label>
                    <textarea name="descricao_produto" id="descricao_produto" rows="3"></textarea>
                </div>
                <div class="form-row">
                    <label>Foto (opcional)</label>
                    <input type="file" name="foto_produto" id="foto_produto" accept="image/*">
                </div>
...
<script>
    ...
    function openAdd(){
        ...
        document.getElementById('foto_produto').value = '';
    ...
    function openEdit(id){
        fetch('produtos.php?json=' + id)
            ...
            .then(data => {
                ...
                document.getElementById('foto_atual').value = data.foto_produto ?? '';
                document.getElementById('nome_produto').value = data.nome_produto ?? '';
                document.getElementById('quantidade_produto').value = data.quantidade_produto ?? 0;
                document.getElementById('preco_produto').value = parseFloat(data.preco_produto ?? 0).toFixed(2);
